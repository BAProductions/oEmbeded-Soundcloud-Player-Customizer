...
class WP_Soundcloud_Player_Customizer{
    ...
    public function wp_soundcloud_player_customizer_init(){
        //require_once(WPSCPC_PLUGIN_DIR . '/wp-soundcloud-player-customizer-settings-manager.php');
    ...
    public function wp_soundcloud_player_customizer_embed($html, $url){
        ...
        if (strpos($url, 'soundcloud.com') !== false) {
            ...
            $visual = (int) get_option('oesc_swap_player', true) ? false : true; // Default value is true
            $show_artwork = (int) get_option('oesc_show_artwork', true) ? true : false; // Default value is true
            $play_button_color = str_replace('#', '', get_option('oesc_play_button_color', '#FF9900')); // Default value is '#FF9900'
            $auto_play = (int) get_option('oesc_auto_play', false) ? true : false; // Default value is false
            $hide_related = (int) get_option('oesc_hide_related', false) ? true : false; // Default value is false
            $show_comments = (int) get_option('oesc_show_comments', false) ? true : false; // Default value is false
            $show_user = (int) get_option('oesc_show_user', false) ? true : false; // Default value is false
            $show_reposts = (int) get_option('oesc_show_reposts', false) ? true : false; // Default value is false
